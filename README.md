...
> 说明：本仓库基于原版 `wx-cli` 增加了对微信 `3.8.x` 的额外兼容支持，
> 重点覆盖 macOS 下的旧版数据目录、SQLCipher 3 解密/WAL 恢复、`Chat_*` 消息表等差异。
> 当前已在 **macOS WeChat 3.8.7** 上完成验证。
...
本仓库在原版基础上补充了对微信 3.8.x 的兼容支持，主要包括：

- macOS 3.8.x 容器目录与账号根目录识别
- SQLCipher 3 参数探测与 WAL 恢复
- `Chat_*` / `SessionAbstract` / `WCContact` 等旧版 schema 兼容

其中 **macOS WeChat 3.8.7** 已完成实机测试。
